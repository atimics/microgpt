...
if args.warmup_steps < 0:
    parser.error(f"warmup_steps ({args.warmup_steps}) must be non-negative")
...
def save_model(path, state_dict, hyperparams, vocab_mapping, optimizer_step=0, loss_history=None):
    ...
    # Serialize each parameter's flat data and optimizer state
    for name, param in state_dict.items():
        # Convert flat array to 2D nested list for readability
        data_2d = []
        m_2d = []
        v_2d = []
        for i in range(param.nout):
            offset = i * param.nin
            data_2d.append([float(param.data[offset + j]) for j in range(param.nin)])
            m_2d.append([float(param.m[offset + j]) for j in range(param.nin)])
            v_2d.append([float(param.v[offset + j]) for j in range(param.nin)])
...
def load_model(path):
    ...
    for name, weight_data in model_data['weights'].items():
        param = FlatParam.__new__(FlatParam)
        param.nout = weight_data['nout']
        param.nin = weight_data['nin']
        
        # Flatten 2D data back to 1D array
        flat_data = []
        flat_m = []
        flat_v = []
        for i in range(param.nout):
            flat_data.extend(weight_data['data'][i])
            flat_m.extend(weight_data['m'][i])
            flat_v.extend(weight_data['v'][i])
        
        param.data = _DA('d', flat_data)
        param.m = _DA('d', flat_m)
        param.v = _DA('d', flat_v)
        param.grad = _DA('d', bytes(param.nout * param.nin * 8))
